...
namespace Rebel.Framework.Persistence.RdbmsModel.Mapping
{
    ...
    public static class ModelToRdbmsMapper
    {
        ...
        private static void MapVersion(TypedEntity source, NodeVersion destination, Node masterNode, AbstractLookupHelper lookupHelper, AbstractMappingEngine masterMapper)
        {
            ...
            MergeMapCollections(
                ...
                (sourceAttrib, destAttrib) =>
                {
                    ...
                    var definitionAlias = sourceAttrib.AttributeDefinition.Alias;
                    ...
                    {
                        destAttrib.AttributeDefinition = destinationSchema.AttributeDefinitions.SingleOrDefault(x => x.Alias == definitionAlias);
                        if (destAttrib.AttributeDefinition == null)
                        {
                            throw new Exception("Catastrophic (this is a bug): Could not find a definition with alias {0} on schema {1} with id {2}".InvariantFormat(definitionAlias, destinationSchema.Alias, destinationSchema.Id));
                        }
                    }
                });
        ...
        public static void MapAttributeTypeDefinition(Model.Attribution.MetaData.AttributeType source, AttributeType destination, AbstractLookupHelper lookupHelper, AbstractMappingEngine masterMapper)
        {
            ...
            destination.Alias = source.Alias ?? StringExtensions.ToRebelAlias(source.Name);
            ...
            destination.TrySetPropertyFromDirty(toProp => toProp.Alias, source, fromProp => fromProp.Alias, () => source.Alias ?? StringExtensions.ToRebelAlias(source.Name));
        ...
        public static void MapAttribute(TypedAttribute source, Attribute destination, AbstractLookupHelper lookupHelper, AbstractMappingEngine masterMapper)
        {
            ...
            {
                var serializationType = source.AttributeDefinition.AttributeType.SerializationType.DataSerializationType;
                switch (serializationType)
                {
                    ...
                    case DataSerializationTypes.LargeInt:
                        ...
                        if (value.Value != null)
                        {
                            // In case a boolean has made it here as a string, but we know we want a boolean, parse it to a proper bool before changing it to an int
                            bool tried = false;
                            if (serializationType == DataSerializationTypes.Boolean && bool.TryParse(value.Value.ToString(), out tried))
                            {
                                intVal.Value = Convert.ToInt32(tried);
                            }
                            else intVal.Value = Convert.ToInt32(value.Value);                            
                        }
                    ...
                    case DataSerializationTypes.Decimal:
                        ...
                        if (value.Value != null) decimalValue.Value = Convert.ToDecimal(value.Value);
        ...
        public static void MapAttributeDefinitionGroup(AttributeGroup source, AttributeDefinitionGroup destination, AbstractLookupHelper lookupHelper, AbstractMappingEngine masterMapper)
        {
            ...
            destination.Id = (Guid)source.Id.Value;
            destination.Name = source.Name;
            destination.Node.DateCreated = source.UtcCreated;
            destination.Ordinal = source.Ordinal;
        }
        ...
        public static void MapAttributeSchemaDefinition(EntitySchema source, AttributeSchemaDefinition destinationSchema, AbstractLookupHelper lookupHelper, AbstractMappingEngine masterMapper)
        {
            ...
            destinationSchema.Node.DateCreated = source.UtcCreated;
            ...
            MapRelations(destinationSchema.Node, source, lookupHelper, masterMapper);
        }
