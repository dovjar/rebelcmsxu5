...
namespace Rebel.Framework.Persistence.Model.Associations._Revised
{
    /// <summary>
    ...
    {
        private IRelatableEntity _collectionOwner;
        ...
        internal void ResetFromCollection(RelationProxyCollection otherCollection)
        {
            using (new WriteLockDisposable(_locker))
            {
                _innerParents.Clear();
                otherCollection._innerParents.ForEach(x => _innerParents.Add(x));
                _innerChildren.Clear();
                otherCollection._innerChildren.ForEach(x => _innerChildren.Add(x));
                LazyLoadDelegate = otherCollection.LazyLoadDelegate;
                _lazyLoadDelegateCalled = otherCollection._lazyLoadDelegateCalled;

                // Don't set the collection owner, as this method would typically be used when cloning so we don't want to leave a ref to the clone source
                //_collectionOwner = otherCollection._collectionOwner;
            }
        }
        ...
        /// <summary>
        /// Gets the manually-added parent proxies without invoking the lazy-load mechanism.
        /// </summary>
        /// <returns></returns>
        public IEnumerable<RelationProxy> GetManualParentProxies()
        {
            Func<RelationProxy, bool> predicate = x => x.Status == RelationProxyStatus.ManuallyAdded;
            return _innerParents.Where(predicate);
        }

        /// <summary>
        /// Gets the manually-added child proxies without invoking the lazy-load mechanism.
        /// </summary>
        /// <returns></returns>
        public IEnumerable<RelationProxy> GetManualChildProxies()
        {
            Func<RelationProxy, bool> predicate = x => x.Status == RelationProxyStatus.ManuallyAdded;
            return _innerChildren.Where(predicate);
        }
