...
namespace Rebel.Framework.Persistence.Model.Versioning
{
    public interface IRevision<out T>
        where T : class, IVersionableEntity
    {
        RevisionData MetaData { get; set; }
        T Item { get; }
    }

    [DebuggerDisplay("Revision Id: {MetaData.Id} of Id: {Item.Id}")]
    public class Revision<T> : ICanBeDirty, INotifyPropertyChanged, IRevision<T>
        where T : class, IVersionableEntity
    {
        /// <summary>
        ...
        public static explicit operator Revision<TypedEntity>(Revision<T> item)
        {
            if (TypeFinder.IsTypeAssignableFrom(typeof(TypedEntity), item.Item.GetType()))
            {
                return new Revision<TypedEntity>(item.Item as TypedEntity) { MetaData = item.MetaData };
            }
            throw new InvalidCastException("Cannot convert type '{0}' to TypedEntity".InvariantFormat(item.Item.GetType().Name));
        }
    }
}
