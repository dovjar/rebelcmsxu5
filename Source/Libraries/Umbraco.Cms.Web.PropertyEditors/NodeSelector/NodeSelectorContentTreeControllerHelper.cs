...
namespace Umbraco.Cms.Web.PropertyEditors.NodeSelector
{
    ...
    internal class NodeSelectorContentTreeControllerHelper
    {
        ...
        /// <returns></returns>
        internal UmbracoTreeResult GetTreeData(
            HiveId parentId, 
            ...
            Func<UmbracoTreeResult> returnResult,
            Func<HiveId, FormCollection, UmbracoTreeResult> returnDefaultResult)
        {
        ...
        internal SelectedItemModel GetNodeSelectorItemModel(HiveId id, Guid treeId, Func<UrlHelper> url)
        {
            ...
            if (!string.IsNullOrEmpty(icon))
            {
                if (icon.Contains("."))
                {
                    model.NodeIcon =
                        url().Content(
                            _backOfficeRequestContext.Application.Settings.UmbracoFolders.
