...
(function ($) {
    ...
    RebelCms.PropertyEditors.RichTextBox = function($elem, o) {
        ...
        var _opts = $.extend({
                ...
                buttons: "newdocument,|,bold,italic,underline,strikethrough,|,justifyleft,justifycenter,justifyright,justifyfull,RebelCmsstyles,styleselect,formatselect,fontselect,fontsizeselect,|,cut,copy,paste,pastetext,pasteword,|,search,replace,|,bullist,numlist,|,outdent,indent,blockquote,|,undo,redo,|,RebelCmslink,link,unlink,RebelCmsanchor,anchor,image,cleanup,help,code,|,insertdate,inserttime,preview,|,forecolor,backcolor,|,tablecontrols,|,hr,removeformat,visualaid,|,sub,sup,|,RebelCmscharmap,charmap,emotions,iespell,media,advhr,|,print,|,ltr,rtl,|,fullscreen,|,insertlayer,moveforward,movebackward,absolute,|,styleprops,|,cite,abbr,acronym,del,ins,attribs,|,visualchars,nonbreaking,pagebreak,|,RebelCmsmacro,RebelCmsmedia",
                textareaId: $elem.attr("id"),
                //these are all of the plugisn that tiny mce has, can be overriden by options
                plugins: "pagebreak,style,layer,table,advhr,advimage,advlink,emotions,iespell,inlinepopups,insertdatetime,preview,media,searchreplace,print,paste,directionality,fullscreen,noneditable,visualchars,nonbreaking,xhtmlxtras,advlist,RebelCmslink,RebelCmsanchor,RebelCmsstyles,RebelCmscharmap,RebelCmsmacro,RebelCmssave,RebelCmsmedia",
                width: 500,
                height: 500,
                stylesheets: "",
                styles: "",
        ...
        var propertyEditor = new RebelCms.PropertyEditors.PropertyEditor(_opts.propertyId);

        //the tiny mce config that gets created
        var config = { };

        //register this property editor
        RebelCms.PropertyEditors.PropertyEditorManager.getInstance().registerPropertyEditor(propertyEditor,
            function(ctx) {
        ...
        return {
            ...
            create: function() {
                ...
                $(RebelCms.PropertyEditors.PropertyEditorManager.getInstance()).bind("setFocus", function(e, context) {
                    ...
                    if (context == null || context.id != propertyEditor.context.id) {
                        $(".RebelCmsSkin .mceExternalToolbar[id^='" + propertyEditor.id + "']").hide();
                    }
                });

                //get all of the ui-elements for this property editor that we've registered
                var buttons = RebelCms.PropertyEditors.PropertyEditorManager.getInstance().getPropertyEditorUIElements(propertyEditor.id);
                var buttonAliases = [];
                ...
                config = {
                    ...
                    RebelCms_mce_controller_paths: _opts.tinyMceControllerPaths,
                    //set the current node id
                    RebelCms_curr_node_id: _opts.currNodeId,
                    //set the ajax url to get macro contents as string from
                    RebelCms_macro_contents_ajax_url: _opts.macroContentAjaxUrl,


                    //*********** END CUSTOM UMBRACO OPTIONS/PROPERTIES**************

                    //by default this will be an empty string unless an RebelCms user overrides it. if it is blank it will use TinyMCE's defaults
                    ...
                    setup: function(ed) {
                        var initToolbar = function(ed) {
                            ...
                            if(ed.editorId == propertyEditor.id + "_Value") 
                            {
                                ...
                                var movedToolbarInstance = $("#editorBar .container .RebelCmsSkin").find("*[id^='" + propertyEditor.id + "']")
                                if (movedToolbarInstance.length == 0) {
                                    //we haven't moved the toolbar instance yet, so lets do that now, first create a container
                                    var panelContainer = $("<div class='ui-panel RebelCmsSkin'/>");
                                    //get the tinymce toolbar for this instance
                                    var tinyMceToolbar = $(".RebelCmsSkin .mceExternalToolbar[id^='" + propertyEditor.id + "']")
                                    //now move it
                                    panelContainer.appendTo($("#editorBar .container")).append(tinyMceToolbar);
                                }
                                
                                $(".RebelCmsSkin .mceExternalToolbar[id^='" + propertyEditor.id + "']").show();
                                
                                //tell the prop editor mgr that tiny mce is active
                                RebelCms.PropertyEditors.PropertyEditorManager.getInstance().setFocus(null, propertyEditor);
                            }
                        };
                        ed.onInit.add(function(ed) {
                            ...
                            ed.windowManager.onOpen.add(function(ed) {
                                ...
                                RebelCms.System.WindowManager.getInstance().toggleTopWindowOverlay(true);
                            });
                            ed.windowManager.onClose.add(function(ed) {
                                ///<summary>Adds event handler to the window manager close event</summary>
                                RebelCms.System.WindowManager.getInstance().toggleTopWindowOverlay(false);
                            });
                        });
    ...
    $.fn.RichTextBox = function(o) {
        return $(this).each(function() {
            var rte = new RebelCms.PropertyEditors.RichTextBox($(this), o);
            rte.create();
        });
    };
