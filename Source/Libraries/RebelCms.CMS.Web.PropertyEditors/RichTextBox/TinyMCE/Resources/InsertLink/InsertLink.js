...
(function ($) {
    ...
    RebelCms.Controls.TinyMCE.InsertLink = function() {
        ...
        function setAllAttribs(elm) {
	        ...
	        if (viewModel.localUrl()) {
	            ...
	            dom.setAttrib(elm, 'data-RebelCms-link', 'internal');
	        } else {
	            dom.setAttrib(elm, 'data-RebelCms-link', '');
	        }
        ...
        return {
            init: function(o) {
                ...
                $("#tabs").RebelCmsTabs({
                    ...
                    onTabChange: function (data) {
                        if (data.activeIndex == 0 && !contentTreeInitialized) {
                            $("#content-tree").RebelCmsTreeApi().createJsTree();
                            contentTreeInitialized = true;
                        } else if (data.activeIndex == 1 && !mediaTreeInitialized) {
                            $("#media-tree").RebelCmsTreeApi().createJsTree();
	            ...
	            if (el != null && el.nodeName == "A") {
	                ...
	                // TODO: Check for data-RebelCms-link attribute
	                var flag = ed.dom.getAttrib(el, 'data-RebelCms-link');
	                if(flag == 'internal') {
