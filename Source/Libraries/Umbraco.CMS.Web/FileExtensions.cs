...
namespace Umbraco.Cms.Web
{
    public static class FileExtensions
    {
        ...
        private static string ToFileNameForDisplay(this string fileName)
        {
            ...
            fileName = Regex.Replace(fileName, "(?<![A-Z])([A-Z])", " $1", RegexOptions.Compiled).Trim(); // Insert space before upper case chars (where not preceded by another upper case char)
