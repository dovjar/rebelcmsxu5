...
{
    /// <summary>
    /// A controller factory for the render pipeline of Umbraco 5. This controller factory tries to create a controller with the supplied
    ...
    public class RenderControllerFactory : IFilteredControllerFactory
    {
        ...
        public IController CreateController(RequestContext requestContext, string controllerName)
        {
            Type controllerType = _innerFactory.GetControllerType(requestContext, controllerName) ??
                   _innerFactory.GetControllerType(requestContext, ControllerExtensions.GetControllerName(typeof(UmbracoController)));
        ...
        /// if the controller is not found. Since we want to try creating a controller, and then fall back to <see cref="UmbracoController"/> if one isn't found,
