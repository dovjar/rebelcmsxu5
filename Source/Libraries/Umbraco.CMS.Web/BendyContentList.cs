...
namespace Umbraco.Cms.Web
{
    ...
    public class DynamicContentList : QueryWrapper<dynamic>
    {
        ...
        private IMembershipService<Member> _membershipService;
        ...
        {
            return new DynamicContentEnumerator(_queryProvider.GetEnumerator(), _hiveManager, _membershipService, _publicAccessService, _defaultValueIfNull);
        }
        ...
        {
            return new DynamicContentList(_sourceId, _hiveManager, _membershipService, _publicAccessService, _defaultValueIfNull, _containedIds, _queryProvider.Where<Content>(predicate, substitutions));
        }

        public DynamicContentList WhereCanAccess()
        {
            var currentMember = _membershipService.GetCurrent();
            var filteredIds = _containedIds.FilterWithPublicAccess(currentMember, _publicAccessService).ToList();
            return new DynamicContentList(_sourceId, _hiveManager, _membershipService, _publicAccessService, _defaultValueIfNull, 
                filteredIds);
        }

        public DynamicContentList WhereCanAccess(HiveId memberId)
        {
            var filteredIds = _containedIds.FilterWithPublicAccess(memberId, _publicAccessService);
            return new DynamicContentList(_sourceId, _hiveManager, _membershipService, _publicAccessService, _defaultValueIfNull, 
                filteredIds);
        }

        public DynamicContentList And(string predicate, params object[] substitutions)
        {
            return Where(predicate, substitutions);
        }
        ...
        {
            return BendIfPossible(entity, _hiveManager, _membershipService, _publicAccessService, _defaultValueIfNull);
        }
        ...
        {
            private readonly IMembershipService<Member> _membershipService;
            ...
            public override dynamic Current
            {
                get
                {
                    var bent = BendIfPossible(InnerEnumerator.Current as TypedEntity, HiveManager, _membershipService,
                        _publicAccessService, _defaultValueIfNull);
