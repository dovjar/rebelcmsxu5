        ...
        public ActionResult Search(SearchModel searchModel)
        {
            ...
            if (tree != null)
            {
                using (var controller = tree.Value)
                {
                    if (controller == null)
                    {
                        throw new TypeLoadException("Could not create controller: " + ControllerExtensions.GetControllerName(tree.Metadata.ComponentType));
                    }
                    ...
                    if (searchableTree != null)
                    {
                        ...
                        var result = this.ProxyRequestToController(
                            searchableTree, 
                            x => x.Search(searchModel.SearchText),
                            tree.Metadata,
                            BackOfficeRequestContext.Application.Settings.UmbracoPaths.BackOfficePath,
                            "treeId");
                        return new CustomJsonResult(() => result.RenderedOutput);
                    }
        ...
        protected UmbracoTreeResult GetRootResultFromTreeController(ITree t)
        {
            ...
            if (tree != null)
            {
                using (var controller = tree.Value)
                {
                    ...
                    //the result returned is the string response from the proxied request, now we need to change it to an UmbracoTreeResult
                    var result = this.ProxyRequestToController(
                        controller, 
