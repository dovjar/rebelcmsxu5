        ...
        where TCreateModel : CreateContentModel, new()
    {
        ...
        public virtual ActionResult CreateNewForm(TCreateModel createModel)
        {
            ...
            using (var uow = Hive.Create<IContentStore>())
            {
                ...
                model.Alias = createModel.Name.ToRebelAlias();
        ...
        private void ProcessCreatingTab(TEditorModel model)
        {
            ...
            if (ValueProvider.GetValue("submit.Tab") != null)
            {
                ...
                else if (model.DefinedTabs.Select(x => x.Alias).Contains(model.NewTabName.ToRebelAlias()))
                {
                ...
                else
                {
                    ...
                    model.DefinedTabs.Add(
                        new Tab
                            {
                                Alias = model.NewTabName.ToRebelAlias(),
        ...
        protected virtual void EnsureSelectListData(TEditorModel model)
        {
            ...
            model.IconsBaseUrl = BackOfficeRequestContext.Application.Settings.RebelFolders.DocTypeIconFolder;
            model.ThumbnailsBaseUrl = BackOfficeRequestContext.Application.Settings.RebelFolders.DocTypeThumbnailFolder;

            model.SpriteFileUrls = BackOfficeRequestContext.DocumentTypeIconResolver.Sprites.Select(x =>
                BackOfficeRequestContext.Application.Settings.RebelFolders.DocTypeIconFolder + "/" + x.Value.Name);
        ...
        {
            model.Properties.RemoveWhere(x => !string.IsNullOrWhiteSpace(x.Alias) && x.Alias.StartsWith("system-"));
            model.InheritedProperties.RemoveWhere(x => !string.IsNullOrWhiteSpace(x.Alias) && x.Alias.StartsWith("system-"));
        }
