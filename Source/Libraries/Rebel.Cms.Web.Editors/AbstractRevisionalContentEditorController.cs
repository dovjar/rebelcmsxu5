...
namespace Rebel.Cms.Web.Editors
{
        ...
        where TEditorModel : StandardContentEditorModel
    {
        ...
        protected AbstractRevisionalContentEditorController(IBackOfficeRequestContext requestContext)
            : base(requestContext)
        {
            _currentUser = BackOfficeRequestContext.Application.Security.Users.GetCurrent();
        }
        ...
        public virtual ActionResult EditForm(HiveId? id, HiveId? revisionId)
        {
            ...
            using (var uow = ReadonlyHive.CreateReadonly<IContentStore>())
            {
                ...
                // Ensure update date is applied
                contentViewModel.UtcModified = DateTimeOffset.Now;
        ...
        protected virtual ActionResult ProcessCreate(TEditorModel model, bool isRevisional)
        {
            ...
            using (var uow = Hive.Create<IContentStore>())
            {
                ...
                // Add created by relation
                entity.Item.RelationProxies.EnlistParentById(_currentUser.ProfileId, FixedRelationTypes.CreatedByRelationType);

                // Add modified by relation
                entity.Item.RelationProxies.EnlistParentById(_currentUser.ProfileId, FixedRelationTypes.ModifiedByRelationType);
