...
namespace Umbraco.Framework.Caching
{
    ...
    public class CacheKey //: AbstractEquatableObject<CacheKey>
    {
        ...
        {
            var casted = obj as CacheKey;
            if (ReferenceEquals(casted, null)) return false;

            return casted.AsJson == AsJson;
        }
        ...
        internal static string GetAndRemoveTypeFromJson(string json, out Type type)
        {
            if (json.Contains("║"))
            {
    ...
    public interface IClassKey<out T>
    {
        T Original { get; }
    }

    public class CacheKey<T> : CacheKey, IClassKey<T>
    {
        public CacheKey()
        {
        }
