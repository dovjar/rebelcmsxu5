...
namespace Umbraco.Framework.Dynamics
{
    ...
    public class BendyObject : DynamicNullableValueObject, INotifyPropertyChanged, IEnumerable<BendyObject>
    {
            ...
            : this()
        {
            ...
            // IEnumerable issue: http://issues.umbraco.org/issue/U5-888
            BendyObjectValue = value;
            ...
            else
            {
                ...
                // http://issues.umbraco.org/issue/U5-888
                ...
                {                    
                    ...
                    {
                        BendyObjectValue.Add(new BendyObject(v));
                    }
                }
        ...
        public override bool TryInvokeMember(InvokeMemberBinder binder, object[] args, out object result)
        {
            ...
            var allMethods = GetMethodsAsStringList(__bendyMethods);
            allMethods = string.IsNullOrWhiteSpace(allMethods)
                             ? "This object doesn't have any dynamic methods that can be run."
                             : "Did you mean to call one of these?\n" + allMethods;

            throw new InvalidOperationException(
                "Could not finding matching Signature for '{0}' with {1} parameters. {2}".InvariantFormat(
        ...
        private static IEnumerable<Parameter> ExcludeFirstBendyParameter(DynamicMethod x)
        {
            ...
            var newParams = excludeFirstBendyParameter.First().Type == typeof(BendyObject) ||
                (x.ScopeType != null && excludeFirstBendyParameter.First().Type == x.ScopeType) ||
                (x.ParentScopeType != null && excludeFirstBendyParameter.First().Type == x.ParentScopeType)
                    ? excludeFirstBendyParameter.Skip(1) : excludeFirstBendyParameter;
            return newParams;
        }
        ...
        private bool InvokeDynamicMethodBody(object[] args, out object result, DynamicMethod methodToInvoke)
        {
            ...
            var defaultParams = new [] { (object)this };

            if (methodToInvoke.ScopeType != null && methodToInvoke.ScopeType != typeof(BendyObject))
            {
                // Handle typed extensions
                defaultParams = new [] { GetOriginalItem(methodToInvoke.ScopeType, this) };
            }
            else if(methodToInvoke.ParentScopeType != null && methodToInvoke.ParentScopeType != typeof(BendyObject))
            {
                // Handle DynamicFieldExtensions
                var parent = (BendyObject)GetParent(typeof(BendyObject), this);
                defaultParams = new[] { GetOriginalItem(methodToInvoke.ParentScopeType, parent), ((dynamic)this).__ParentKey};
            }

            if (count == 0)
            {
                result = ((LambdaExpression)methodToInvoke.Body).Compile().DynamicInvoke(defaultParams);
                return true;
            }
            ...
            try
            {
                result = local.Compile().DynamicInvoke(defaultParams.Concat(args).ToArray());
                return true;
            }
        ...
        private static object GetOriginalItem(Type originalItemType, BendyObject bendy)
        {
            if (bendy["__OriginalItem"] != null && bendy["__OriginalItem"].GetType() == originalItemType)
            {
        ...
        private static object GetParent(Type parentType, BendyObject bendy)
        {
            if (bendy["__Parent"] != null && bendy["__Parent"].GetType() == parentType)
            {
                return ((dynamic)bendy).__Parent;
            }

            throw new ApplicationException("Unable to locate Parent in Bendy Object of type " + parentType.ToString());
        }
