...
namespace Rebel.Framework.Persistence.NHibernate
{
    public class SchemaRepository : AbstractSchemaRepository
    {
        ...
        protected override IEnumerable<TEntity> PerformGet<TEntity>(bool allOrNothing, params HiveId[] ids)
        {
            ...
            var entities = Helper.NhSession
                .CreateCriteria(destinationType)
                .Add(Restrictions.In(Projections.Property<Node>(x => x.Id), values))
                .SetCacheable(true)
                .List();
        ...
        public override void PerformDelete<T>(HiveId id)
        {
            Mandate.ParameterNotEmpty(id, "id");
            ...
            var hasNode = nhObject as IHasNode;
            if (hasNode != null && hasNode.Node != null)
            {
                Helper.RemoveRelationsBiDirectional(hasNode.Node);
            }
            var isNode = nhObject as Node;
            if (isNode != null)
            {
                Helper.RemoveRelationsBiDirectional(isNode);
            }
