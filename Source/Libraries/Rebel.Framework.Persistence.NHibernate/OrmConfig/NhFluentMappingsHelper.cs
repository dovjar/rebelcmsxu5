    ...
    public static class NhFluentMappingsHelper
    {
        ...
        private static string GenerateIndexName<TSource, TPropValue>(Expression<Func<TSource, TPropValue>> propertyExpression)
        {
            ...
            if (propExpression != null)
                return GenerateIndexName<TSource>(propExpression.Member.Name);
            throw new InvalidOperationException("Cannot generate an index name unless the expression represents a member");
        }

        public static string GenerateIndexName<TSource>(string indexName)
        {
            return "U5_IDX_" + typeof (TSource).Name + "_" + indexName;
        }
        ...
        public static string GenerateIndexName<TSource>(this ClassMap<TSource> mapper, string indexName)
        {
            return GenerateIndexName<TSource>(indexName);
        }
