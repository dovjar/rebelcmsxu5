...
namespace Umbraco.Hive.Providers.IO
{
    ...
    public class EntityRepository : AbstractEntityRepository
    {
        ...
        internal IEnumerable<Relation> GetParentRelations(HiveId childId)
        {
            ...
            // Get other relations, ensure we yield asap though (not a foreach on the method body)
            var relationsByPattern = GetRelationsByPattern("*-" + child.Id.Value.ToString().ToMd5()).ToArray();
            
            foreach (var relation in relationsByPattern)
            {
