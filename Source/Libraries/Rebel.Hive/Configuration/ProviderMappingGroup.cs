    ...
    public class ProviderMappingGroup : IRequiresFrameworkContext
    {
        ...
        public ProviderUriMatchResult IsMatchForUri(string uri)
        {
            ...
            try
            {
                ...
                {
                    var fixUriBugs = FixUriBugs(incomingUriBuilder.ToString());
                    var matches = WildcardUriMatches
                        .Where(wildcard => fixUriBugs.StartsWith(FixUriBugs(wildcard.Root.ToString()).TrimEnd('*'), StringComparison.InvariantCultureIgnoreCase))
                        .Select(match => new ProviderUriMatchResult(true, match))
                        .FirstOrDefault();
