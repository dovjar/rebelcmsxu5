...
using RebelCms.Framework.DataManagement.Linq.QueryModel;
using RebelCms.Framework.DataManagement.Linq.ResultBinding;
...
namespace RebelCms.Framework.Persistence.XmlStore.DataManagement
{
    public class DataContext : AbstractDataContext, IQueryableDataSource
    {
        ...
        internal XElement FindItemByUrl(string url)
        {
            ...
            if (xmlElement != null)
            {
                return FindWithRebelCmsRedirect(xmlElement);
            }
            ...
            return xmlElement != null ? FindWithRebelCmsRedirect(xmlElement) : null;
        }
        ...
        internal XElement FindPageByUrlAlias(string alias)
        {
            string expression = string.Concat(
                "/root//*[string-length(@id)>0 and data[@alias='RebelCmsUrlAlias']/text()=\"", alias, "\"]");
            return FindElementByExpression(expression);
        }
        ...
        internal XElement FindWithRebelCmsRedirect(XElement pageXml)
        {
            IEnumerable<XElement> redirectElements = FindElementsByExpression("./data[@alias='RebelCmsRedirect']");
