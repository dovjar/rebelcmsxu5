    ...
    public abstract class AbstractFileEditorController : StandardEditorController
    {
        ...
        public virtual ActionResult CreateNewForm(CreateFileModel createModel)
        {
            ...
            using (var uow = Hive.Create())
            {
                    ...
                    var folder = new File()
                        {
                            ...
                            RootedPath = createModel.ParentId == FixedHiveIds.SystemRoot
                                ? createModel.Name.ToRebelCmsAlias(removeSpaces: true)
                                : (string)createModel.ParentId.Value + "/" + createModel.Name.ToRebelCmsAlias(removeSpaces: true)
                        };
                ...
                model.Name = createModel.Name.ToRebelCmsAlias(removeSpaces:true) + createModel.FileExtension;
        ...
        {
            var controllerName = RebelCmsController.GetControllerName(this.GetType());

            // Update thumbnails
            var thumbnailFolder = Url.Content(BackOfficeRequestContext.Application.Settings.RebelCmsFolders.DocTypeThumbnailFolder);
