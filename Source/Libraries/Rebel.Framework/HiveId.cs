...
namespace Rebel.Framework
{
    ...
    public struct HiveId : IEquatable<HiveId>
    {
        ...
        /// <summary>
        /// Performs an explicit conversion from <see cref="System.Guid"/> to <see cref="Rebel.Framework.HiveId"/>.
        /// </summary>
        ...
        /// <summary>
        /// Performs an explicit conversion from <see cref="System.Int32"/> to <see cref="Rebel.Framework.HiveId"/>.
        /// </summary>
        /// <param name="value">The value.</param>
        /// <returns>The result of the conversion.</returns>
        /// <remarks></remarks>
        ...
        /// <summary>
        /// Parses the specified formatted value if provided as an object.
        /// </summary>
        /// <param name="formattedValue">The formatted value.</param>
        /// <returns></returns>
        public static HiveId Parse(object formattedValue)
        {
            // Plenty of devs have ended up using HiveId.Parse passing in dynamic objects which was
            // working fine when the type of the dynamic was convertible to string but if they are
            // of type Lazy<object> which can sometimes happen during the Bend method in the Cms.Web assembly
            // then the view compiler couldn't pick an overload, so do a few bits of type checking here
            // so that the view can pick this overload of Parse and we'll handle the rest.
            if (formattedValue == null) return HiveId.Empty;

            var tryCastString = formattedValue as string;
            if (!tryCastString.IsNullOrWhiteSpace()) return Parse(tryCastString);
            var tryCastLazyString = formattedValue as Lazy<string>;
            if (tryCastLazyString != null) return Parse(tryCastLazyString.Value);
            var tryCastLazyObject = formattedValue as Lazy<object>;
            if (tryCastLazyObject != null)
            {
                tryCastString = tryCastLazyObject.Value as string;
                if (!tryCastString.IsNullOrWhiteSpace()) return Parse(tryCastString);
            }
            var tryCastLazyDynamic = formattedValue as Lazy<dynamic>;
            if (tryCastLazyDynamic != null)
            {
                tryCastString = tryCastLazyDynamic.Value as string;
                if (!tryCastString.IsNullOrWhiteSpace()) return Parse(tryCastString);
            }

            var asObject = ((object) formattedValue);
            var getType = asObject.GetType().FullName;
            throw new FormatException("Cannot parse '{0}' which is of type '{1}' as a HiveId".InvariantFormat(asObject, getType));
        }
