...
namespace Umbraco.Framework.Persistence.NHibernate
{
    public class RevisionRepository : AbstractRevisionRepository<TypedEntity>
    {
        ...
        {
            var agg = Helper.GenerateAggregateStatusQuery(new[] {(Guid) entityId.Value}, revisionStatusType, true);
            var revision = Helper.GetNodeVersionsInBatches(agg.GeneratedQuery, false).FirstOrDefault();
        ...
        public override IEnumerable<Revision<TEntity>> PerformGetLatestRevisions<TEntity>(bool allOrNothing, RevisionStatusType revisionStatusType = null, params HiveId[] entityIds)
        {
            ...
            var agg = Helper.GenerateAggregateStatusQuery(nodeIds, revisionStatusType, true);
            var revisions = Helper.GetNodeVersionsInBatches(agg.GeneratedQuery, false);
