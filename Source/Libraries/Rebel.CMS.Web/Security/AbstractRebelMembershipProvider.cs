...
namespace Rebel.Cms.Web.Security
{
    ...
    public abstract class AbstractRebelMembershipProvider : MembershipProvider
    {
        ...
        {
            get { return _appContext ?? (_appContext = DependencyResolver.Current.GetService<IRebelApplicationContext>()); }
        ...
        /// </summary>
        public AbstractRebelMembershipProvider()
        {
        }
        ...
        private void UpdateScopedCache(string username, RebelMembershipUser user, object newValue)
        {
            AppContext.FrameworkContext.ScopedCache.AddOrChange(GetRebelUserCacheKeyForUsername(username), s => newValue);
            AppContext.FrameworkContext.ScopedCache.AddOrChange(GetRebelUserCacheKeyForId(user.Id), s => newValue);
        }
                                                                  ...
                                                                  out int totalRecords)
        {
            ...
            Expression<Func<RebelMembershipUser, bool>> whereExpression = null;
            ...
            using (var uow = Hive.Create())
            {
                var queryUsers = uow.Repositories.Query<RebelMembershipUser>().WithParentIds(VirtualRootId).Where(whereExpression);

                totalRecords = queryUsers.Count();

                // Only bother querying if count is > 0
                var foundUsers = new List<RebelMembershipUser>();
                if (totalRecords > 0)
                {
        ...
        {
            //change the where clause depending on if there are wildcard chars specified
            Func<RebelMembershipUser, bool> whereClause = u =>
                {
            ...
            using (var uow = Hive.Create())
            {
                ...
                var users = uow.Repositories
                    .GetChildren<RebelMembershipUser>(FixedRelationTypes.DefaultRelationType, VirtualRootId)
        ...
        {
            var hiveUser = GetRebelUser(username, userIsOnline);
        ...
        public override MembershipUser GetUser(object providerUserKey, bool userIsOnline)
        {
            ...
            var hiveUser = GetRebelUser(hiveId.Result, userIsOnline);
            return hiveUser != null ? ConvertUserToMembershipUser(hiveUser) : null;
        }
        ...
        public override void UpdateUser(MembershipUser membershipUser)
        {
            using (var uow = Hive.Create())
            {
                var user = GetRebelUser(uow, (HiveId)membershipUser.ProviderUserKey, false);
        ...
        #region Rebel Extensions

        public RebelMembershipUser GetRebelUser(string username, bool userIsOnline)
        {
            using (var uow = Hive.Create())
            {
                return GetRebelUser(uow, username, userIsOnline);
            }
        }
        ...
        private string GetRebelUserCacheKeyForUsername(string username)
        {
            return "hmp_GetRebelUser_" + VirtualRootId.ToString().ToMd5() + "_" + username;
        }

        public RebelMembershipUser GetRebelUser(HiveId id, bool userIsOnline)
        {
            using (var uow = Hive.Create())
            {
                return GetRebelUser(uow, id, userIsOnline);
            }
        }
        ...
        /// <param name="password">The password.</param>
        /// <returns></returns>
        private bool ValidateUserInternal(RebelMembershipUser user, string password)
        {
        ...
        /// <returns></returns>
        private MembershipUser ConvertUserToMembershipUser(RebelMembershipUser user)
        {
        ...
        /// <returns></returns>
        private MembershipUserCollection ConvertUserListToMembershipCollection(IEnumerable<RebelMembershipUser> users)
        {
            var returnCollection = new MembershipUserCollection();

            foreach (RebelMembershipUser user in users)
            {
