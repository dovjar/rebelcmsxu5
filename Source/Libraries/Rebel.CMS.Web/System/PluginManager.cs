...
namespace Rebel.Cms.Web.System
{
    ...
    public class PluginManager
    {
        ...
        {
            var filePath = Path.Combine(_pluginFolder.FullName, "Cache", "rebel-plugins.hash");
        ...
        {
            var filePath = Path.Combine(pluginFolder.FullName, "Cache", "rebel-plugins.list");
            if (!File.Exists(filePath))
        ...
        private static void WriteCachePluginsHash(IEnumerable<FileInfo> plugins)
        {
            ...
            File.WriteAllText(Path.Combine(dir.FullName, "rebel-plugins.hash"), GetPluginsHash(plugins), Encoding.UTF8);
        }
        ...
        private static void WriteCachePluginsList(IEnumerable<FileInfo> plugins)
        {
            ...
            var dir = Directory.CreateDirectory(Path.Combine(_pluginFolder.FullName, "Cache"));
            File.WriteAllText(Path.Combine(dir.FullName, "rebel-plugins.list"), sb.ToString(), Encoding.UTF8);
            ...
            File.WriteAllText(Path.Combine(_shadowCopyFolder.FullName, "rebel-plugins-list.txt"),
                "This file lists all of the Rebel plugin DLL files that have been shadow copied to this folder.\r\n" 
                + sb.ToString(), 
                Encoding.UTF8);
        }
        ...
        private static Assembly PerformFileDeployAndRegistration(FileInfo plug, bool performShadowCopy)
        {
            if (plug.Directory.Parent == null)
                throw new InvalidOperationException("The plugin directory for the " + plug.Name +
                                                    " file exists in a folder outside of the allowed Rebel folder heirarchy");
