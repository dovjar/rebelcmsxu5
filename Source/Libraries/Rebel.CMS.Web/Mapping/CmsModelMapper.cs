...
using ParameterEditorModel = Rebel.Cms.Web.Model.BackOffice.ParameterEditors.EditorModel;
using PropertyEditorModel = Rebel.Cms.Web.Model.BackOffice.PropertyEditors.EditorModel;
...
namespace Rebel.Cms.Web.Mapping
{
    ...
    public sealed class CmsModelMapper : AbstractFluentMappingEngine
    {
                ...
                .CreateUsing(x =>
                    {
                        ...
                        if (string.IsNullOrEmpty(x.Alias)) x.Alias = x.Name.Value.ToRebelAlias();
        ...
        public void ConfigureModelToPersistenceMappings()
        {
            ...
            this.CreateMap(new ContentEditorModelToTypedEntity<UserEditorModel, User>(this, _resolverContext))
                .ForMember(x => x.LastLoginDate, x => x.MapFrom(y => y.LastLoginDate.HasValue ? new DateTimeOffset(y.LastLoginDate.Value) : DateTimeOffset.MinValue))
                .ForMember(x => x.LastActivityDate, x => x.MapFrom(y => y.LastActivityDate.HasValue ? new DateTimeOffset(y.LastActivityDate.Value) : DateTimeOffset.MinValue))
                .ForMember(x => x.LastPasswordChangeDate, x => x.MapFrom(y => y.LastPasswordChangeDate.HasValue ? new DateTimeOffset(y.LastPasswordChangeDate.Value) : DateTimeOffset.MinValue));

            #endregion

            #region MemberEditorModel -> Member

            this.CreateMap(new ContentEditorModelToTypedEntity<MemberEditorModel, Member>(this, _resolverContext))
                 .ForMember(x => x.LastLoginDate, x => x.MapFrom(y => y.LastLoginDate.HasValue ? new DateTimeOffset(y.LastLoginDate.Value) : DateTimeOffset.MinValue))
                .ForMember(x => x.LastActivityDate, x => x.MapFrom(y => y.LastActivityDate.HasValue ? new DateTimeOffset(y.LastActivityDate.Value) : DateTimeOffset.MinValue))
                .ForMember(x => x.LastPasswordChangeDate, x => x.MapFrom(y => y.LastPasswordChangeDate.HasValue ? new DateTimeOffset(y.LastPasswordChangeDate.Value) : DateTimeOffset.MinValue));
            ...
            #region ProfileTypeEditorModel -> EntitySchema

            this.CreateMap(new SchemaEditorModelToEntitySchema<ProfileTypeEditorModel>(this, _resolverContext));
            ...
            #region PackageDefinitionEditorModel -> PackageDefinition

            this.CreateMap<PackageDefinitionEditorModel, PackageDefinition>();
