    ...
    public class PackageAreaRegistration : AreaRegistration
    {
        ...
        public override void RegisterArea(AreaRegistrationContext context)
        {
            var editorControllersForPackage = _editorControllers
                .Select(x => x.Metadata)
                .Where(x => !x.IsInternalRebelEditor)
            ...
            var standardRoute = context.Routes.MapRoute(
                AreaName + "-default",
                _applicationContext.Settings.RebelPaths.BackOfficePath + "/" + AreaName + "/{controller}/{action}/{id}",
            ...
            standardRoute.DataTokens.Add("rebel", "custom"); //ensure the rebel token is set (used for view engine perf)
        }
