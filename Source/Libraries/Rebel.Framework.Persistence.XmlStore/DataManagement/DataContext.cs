...
using Rebel.Framework.DataManagement.Linq.QueryModel;
using Rebel.Framework.DataManagement.Linq.ResultBinding;
...
namespace Rebel.Framework.Persistence.XmlStore.DataManagement
{
    public class DataContext : AbstractDataContext, IQueryableDataSource
    {
        ...
        internal XElement FindItemByUrl(string url)
        {
            ...
            if (xmlElement != null)
            {
                return FindWithRebelRedirect(xmlElement);
            }
            ...
            return xmlElement != null ? FindWithRebelRedirect(xmlElement) : null;
        }
        ...
        internal XElement FindPageByUrlAlias(string alias)
        {
            string expression = string.Concat(
                "/root//*[string-length(@id)>0 and data[@alias='rebelUrlAlias']/text()=\"", alias, "\"]");
            return FindElementByExpression(expression);
        }
        ...
        internal XElement FindWithRebelRedirect(XElement pageXml)
        {
            IEnumerable<XElement> redirectElements = FindElementsByExpression("./data[@alias='rebelRedirect']");
