...
using Hostname = Rebel.Cms.Web.Trees.MenuItems.Hostname;
...
namespace Rebel.Cms.Web.Trees
{
        ...
        //, ISearchableTree
    {
        ...
        public override RebelTreeResult Index(HiveId id, FormCollection querystrings)
        {
        ...
        protected override HiveId RootNodeId
        {
            get
            {
                ...
                _rootNodeId = FixedHiveIds.ContentVirtualRoot;
                ...
                if (Request.IsAuthenticated && HttpContext.User.Identity is RebelBackOfficeIdentity)
                {
                    ...
                    if (currentIdentity.StartContentNode != HiveId.Empty)
                        _rootNodeId = currentIdentity.StartContentNode;
                }
                ...
                using (var uow = hiveProvider.CreateReadonly<IContentStore>())
                {
                    ...
                    using (DisposableTimer.TraceDuration<ContentTreeController>("FilterWithPermissions", "FilterWithPermissions"))
                    {
                        childIds = childIds.FilterWithPermissions(
                                BackOfficeRequestContext.Application.Security.Permissions,
                                ((RebelBackOfficeIdentity)User.Identity).Id,
                                uow,
                                securityUow,
        ...
        protected override IEnumerable<TreeNode> GetChildTreeNodes(HiveId parentId, FormCollection queryStrings)
        {
            ...
            using (var uow = hiveProvider.CreateReadonly<IContentStore>())
            {
                ...
                using (DisposableTimer.TraceDuration<ContentTreeController>("FilterWithPermissions", "FilterWithPermissions"))
                {
                    childIds = childIds.FilterWithPermissions(
                            BackOfficeRequestContext.Application.Security.Permissions,
                            ((IRebelIdentity)User.Identity).Id,
                            uow,
                            securityUow,
                            new ViewPermission().Id
                            ).ToArray();
