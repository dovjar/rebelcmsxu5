...
{
    /// <summary>
    /// Custom route handler to assign the correct controller based on the RebelCms request
    /// </summary>
    public class RenderRouteHandler : IRouteHandler
    {
        ...
        /// <summary>
        /// Assigns the correct controller based on the RebelCms request and returns a standard MvcHandler to prcess the response,
        ...
        public IHttpHandler GetHttpHandler(RequestContext requestContext)
        {
            ...
            requestContext.RouteData.DataTokens.Add("RebelCms", renderModel);
        ...
        /// <summary>
        /// Handles a posted form to an RebelCms Url and ensures the correct controller is routed to and that
        ...
        protected virtual IHttpHandler HandlePostedValues(RequestContext requestContext, PostedDataProxyInfo postedInfo, IRebelCmsRenderModel renderModel, RouteDefinition routeDefinition)
        {
            ...
            requestContext.RouteData.DataTokens["RebelCms-item-url"] = requestContext.HttpContext.Request.Url.AbsolutePath;
            //store the original route definition
            requestContext.RouteData.DataTokens["RebelCms-route-def"] = routeDefinition;
        ...
        /// <param name="renderModel"></param>
        /// <returns></returns>
        protected virtual RouteDefinition GetRebelCmsRouteDefinition(RequestContext requestContext, IRebelCmsRenderModel renderModel)
        {
            var def = new RouteDefinition
                {
                    ControllerName = RebelCmsController.GetControllerName<RebelCmsController>(), 
                    Controller = new RebelCmsController(),
            ...
            if (renderModel.CurrentNode != null && renderModel.CurrentNode.CurrentTemplate != null)
            {
                using (var uow = _applicationContext.Hive.OpenReader<IFileStore>(new Uri("storage://templates")))
                {
                    ...
                    {                        
                        ...
                        //ensure the controller is of type 'RebelCmsController'
                        if (!(controller is RebelCmsController))
                        {
                            LogHelper.Warn<RenderRouteHandler>("The current Document Type {0} matches a locally declared controller of type {1}. Custom Controllers for RebelCms routing must inherit from '{2}'.", renderModel.CurrentNode.ContentType.Alias, controller.GetType().FullName, typeof(RebelCmsController).FullName);
                        ...
                        def.ControllerName = RebelCmsController.GetControllerName(controller.GetType());

                        // Template might be null if none is assigned to the node
                        if (template != null)
                        {
                            //check if the custom controller has an action with the same name as the template name (we convert ToRebelCmsAlias since the template name might have invalid chars).
                            //NOTE: This also means that all custom actions MUST be PascalCase.. but that should be standard.
                            var templateName = template.Name.Split('.')[0].ToRebelCmsAlias(StringAliasCaseType.PascalCase);
        ...
        /// <param name="requestContext"></param>
        /// <param name="renderModel"></param>
        protected internal IHttpHandler GetHandlerForRoute(RequestContext requestContext, IRebelCmsRenderModel renderModel)
        {
            var routeDef = GetRebelCmsRouteDefinition(requestContext, renderModel);

            //Need to check for a special case if there is form data being posted back to an RebelCms URL
