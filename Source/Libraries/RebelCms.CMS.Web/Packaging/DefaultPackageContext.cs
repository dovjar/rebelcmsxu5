...
namespace RebelCms.Cms.Web.Packaging
{
    public class DefaultPackageContext : IPackageContext
    {
        ...
        public DefaultPackageContext(RebelCmsSettings settings, Func<string, string> mapPath)
        {
            _settings = settings;

            _pluginInstallFolderPath = mapPath(_settings.PluginConfig.PluginsPath + "/Packages");
            _localPackageRepoFolderPath = mapPath(_settings.RebelCmsFolders.LocalPackageRepositoryFolder);
