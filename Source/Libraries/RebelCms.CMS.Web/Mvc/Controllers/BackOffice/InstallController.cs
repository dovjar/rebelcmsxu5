...
using RebelCms.Cms.Web.Model.Install;
...
{
    [RebelCmsInstallAuthorizeAttribute]
    ...
    public class InstallController : Controller
    {
        ...
        public InstallController(IBackOfficeRequestContext requestContext)
        {
            ...
            ActionInvoker = new RebelCmsInstallActionInvoker(requestContext);
        ...
        public ActionResult DatabaseForm(DatabaseInstallModel model)
        {
            ...
            var configFile = new FileInfo(
                Path.Combine(HttpContext.Server.MapPath("~/App_Data/RebelCms/HiveConfig"), "web.config"));
        ...
        public ActionResult CreateUserForm(CreateNewUserModel model)
        {
            ...
            if(model.SignUpForNewsletter)
            {
                ...
                {
                    new WebClient()
                        .UploadValues("http://RebelCms.org/base/Ecom/SubmitEmail/installer.aspx", new NameValueCollection()
                    {
                        {
                            "name",
                            model.Name
        ...
        {
            this.HttpContext.RebelCmsLogout();
