    ...
    public class PackageAreaRegistration : AreaRegistration
    {
        ...
        public override void RegisterArea(AreaRegistrationContext context)
        {
            var editorControllersForPackage = _editorControllers
                .Select(x => x.Metadata)
                .Where(x => !x.IsInternalRebelCmsEditor)
            ...
            var treeControllersForPackage = _treeControllers
                .Select(x => x.Metadata)
                .Where(x => !x.IsInternalRebelCmsTree)
                .Where(x => x.PluginDefinition != null)
                .Where(x => x.PluginDefinition.PackageFolderPath == _packageFolder.FullName)
                .ToArray();
            ...
            var standardRoute = context.Routes.MapRoute(
                AreaName + "-default",
                _applicationContext.Settings.RebelCmsPaths.BackOfficePath + "/" + AreaName + "/{controller}/{action}/{id}",
            ...
            standardRoute.DataTokens.Add("RebelCms", "custom"); //ensure the RebelCms token is set (used for view engine perf)
        }
