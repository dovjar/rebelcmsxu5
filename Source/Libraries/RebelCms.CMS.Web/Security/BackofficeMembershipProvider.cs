...
namespace RebelCms.Cms.Web.Security
{
    public class BackOfficeMembershipProvider : MembershipProvider
    {
        ...
        {
            _appContext = DependencyResolver.Current.GetService<IRebelCmsApplicationContext>();
        ...
        public override bool ChangePassword(string username, string oldPassword, string newPassword)
        {
            ...
            {
                var user = GetRebelCmsUser(_appContext, uow, username, false);
        ...
        {
            var hiveUser = GetRebelCmsUser(username, userIsOnline);
        ...
        public override MembershipUser GetUser(object providerUserKey, bool userIsOnline)
        {
            ...
            var hiveUser = GetRebelCmsUser(hiveId.Result, userIsOnline);
            return hiveUser != null ? ConvertUserToMembershipUser(hiveUser) : null;
        }
        ...
        public override void UpdateUser(MembershipUser membershipUser)
        {
            using (var uow = _hive.Create())
            {
                var user = GetRebelCmsUser(uow, (HiveId)membershipUser.ProviderUserKey, false);
        ...
        public override void Initialize(string name, NameValueCollection config)
        {
            ...
            if (string.IsNullOrEmpty(config["description"]))
            {
                ...
                config.Add("description", "RebelCms backoffice membership provider");
            }
        ...
        #region RebelCms Extensions

        public User GetRebelCmsUser(string username, bool userIsOnline)
        {
            using (var uow = _hive.Create())
            {
                return GetRebelCmsUser(_appContext, uow, username, userIsOnline);
            }
        }

        internal static User GetRebelCmsUser(IRebelCmsApplicationContext appContext, IGroupUnit<ISecurityStore> uow, string username, bool userIsOnline)
        {
        ...
        public User GetRebelCmsUser(HiveId id, bool userIsOnline)
        {
            using (var uow = _hive.Create())
            {
                return GetRebelCmsUser(uow, id, userIsOnline);
            }
        }

        internal static User GetRebelCmsUser(IGroupUnit<ISecurityStore> uow, HiveId id, bool userIsOnline)
        {
            var user = uow.Repositories
                .GetEntityByRelationType<User>(FixedRelationTypes.DefaultRelationType, FixedHiveIds.UserVirtualRoot)
