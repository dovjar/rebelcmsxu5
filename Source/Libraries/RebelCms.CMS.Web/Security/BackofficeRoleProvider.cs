    ...
    public class BackOfficeRoleProvider : RoleProvider
    {
        ...
        public BackOfficeRoleProvider(Lazy<IHiveManager> hiveManager)
        {
            ...
            _hiveManager = hiveManager ?? new Lazy<IHiveManager>(
                    () => DependencyResolver.Current.GetService<IRebelCmsApplicationContext>().Hive);
        ...
        public override string[] GetRolesForUser(string username)
        {
            ...
            if (Thread.CurrentPrincipal.Identity is RebelCmsBackOfficeIdentity)
            {
                var identity = (RebelCmsBackOfficeIdentity)Thread.CurrentPrincipal.Identity;
        ...
        public override bool IsUserInRole(string username, string roleName)
        {
            ...
            if (Thread.CurrentPrincipal.Identity is RebelCmsBackOfficeIdentity)
            {
                return ((RebelCmsBackOfficeIdentity)Thread.CurrentPrincipal.Identity).Roles.Contains(roleName);
            }
        ...
        public override void Initialize(string name, NameValueCollection config)
        {
            ...
            if (string.IsNullOrEmpty(config["description"]))
            {
                ...
                config.Add("description", "RebelCms backoffice role provider");
            }
        ...
        /// <summary>
        /// This checks the current identity is a RebelCmsBackOfficeIdentity, if so , it updates its roles and re-sets the cookie
        ...
        private static void RemoveRolesFromCurrentIdentity(IEnumerable<string> roleNames, IEnumerable<string> userNames = null)
        {
            ...
            if (!(Thread.CurrentPrincipal.Identity is RebelCmsBackOfficeIdentity)) return;
            var identity = (RebelCmsBackOfficeIdentity)Thread.CurrentPrincipal.Identity;
            if (userNames == null || userNames.Contains(identity.Name))
            {
                ...
                if (HttpContext.Current != null)
                {
                    ...
                    wrapper.CreateRebelCmsAuthTicket(new UserData
                        {
