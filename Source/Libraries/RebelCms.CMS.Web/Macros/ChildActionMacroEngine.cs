...
namespace RebelCms.Cms.Web.Macros
{
    ...
    { 
            ...
            IRoutableRequestContext routableRequestContext)
        {
            ...
            {
                if (controller == null)
                    throw new TypeLoadException("Could not create controller: " + RebelCmsController.GetControllerName(surfaceController.Metadata.ComponentType));

                //need to get the macroParams to put into an array
                var actionParams = macroParams.Select(i => i.Value).ToArray();

                var area = routableRequestContext.Application.Settings.RebelCmsPaths.BackOfficePath;
