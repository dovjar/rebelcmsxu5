...
namespace RebelCms.Cms.Web
{
    ...
    {
        public static readonly Regex TagMatch = new Regex(@"(<.*?data-RebelCms-link=""internal"".*?>)", RegexOptions.Compiled);
        ...
        public static readonly Regex RebelCmsSrcMatch = new Regex(@"data-RebelCms-src=""(?<Src>.*?)""", RegexOptions.Compiled);
            ...
            Func<HiveId, string> linkFoundCallback) 
        {
            var s = TagMatch.Replace(input, x =>
            {
                ...
                if(HrefMatch.IsMatch(strLink))
                {
                    ...
                    strLink = strLink.Replace(@"data-RebelCms-link=""internal""", "");
                }
                // Media
                else if (SrcMatch.IsMatch(strLink) && RebelCmsSrcMatch.IsMatch(strLink))
                {
                    var hiveId = new HiveId(RebelCmsSrcMatch.Match(strLink).Groups["Src"].Value);
                    ...
                    if(!url.IsNullOrWhiteSpace())
                    {
                        ...
                        strLink = RebelCmsSrcMatch.Replace(strLink, "");
                        strLink = strLink.Replace(@"data-RebelCms-link=""internal""", "");
                    }
                    else
                    {
