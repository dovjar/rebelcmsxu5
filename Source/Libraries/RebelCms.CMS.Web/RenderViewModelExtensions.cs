...
using RebelCms.Framework.Dynamics.Expressions;
        ...
        public static dynamic Bend(this Content content, IHiveManager hiveManager)
        {
                ...
                content.Attributes.Where(typedAttribute => typedAttribute.Values.Any()))
            {
                ...
                if (typedAttribute.Values.Count() > 1)
                {
                    ...
                    {
                        subBendy1[v.Key.ToRebelCmsAlias(StringAliasCaseType.PascalCase)] = v.Value;
                        subBendy2[v.Key.ToRebelCmsAlias(StringAliasCaseType.Unchanged)] = v.Value;
                    }

                    bendy[typedAttribute.AttributeDefinition.Alias.ToRebelCmsAlias(StringAliasCaseType.PascalCase)] = subBendy1;
                    bendy[typedAttribute.AttributeDefinition.Alias.ToRebelCmsAlias(StringAliasCaseType.Unchanged)] = subBendy2;
                }
                else
                {
                    bendy[typedAttribute.AttributeDefinition.Alias.ToRebelCmsAlias(StringAliasCaseType.PascalCase)] = typedAttribute.Values.First().Value;
                    bendy[typedAttribute.AttributeDefinition.Alias.ToRebelCmsAlias(StringAliasCaseType.Unchanged)] = typedAttribute.Values.First().Value;
                }
            }
            ...
            var nodeNameAlias = NodeNameAttributeDefinition.AliasValue.ToRebelCmsAlias(StringAliasCaseType.PascalCase);
            try
            {
