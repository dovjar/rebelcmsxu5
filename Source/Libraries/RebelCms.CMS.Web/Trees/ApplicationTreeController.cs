    ...
    public class ApplicationTreeController : SecuredBackOfficeController
    {
        ...
        public virtual ActionResult Index(string appAlias)
        {
            ...
            if (application == null)
            {
                ...
                return new RebelCmsTreeResult(new TreeNodeCollection(), ControllerContext);
            }
            ...
            return new RebelCmsTreeResult(NodeCollection, ControllerContext);
        ...
        public TreeSearchJsonResult Search(SearchModel searchModel)
        {
            ...
            if (tree != null)
            {
                using (var controller = tree.Value)
                {
                    if (controller == null)
                    {
                        throw new TypeLoadException("Could not create controller: " + RebelCmsController.GetControllerName(tree.Metadata.ComponentType));
                    }
        ...
        /// <param name="t"></param>
        /// <returns></returns>
        protected RebelCmsTreeResult GetRootResultFromTreeController(ITree t)
        {
            var tree = BackOfficeRequestContext.RegisteredComponents
            ...
            if (tree != null)
            {
                using (var controller = tree.Value)
                {
                    if (controller == null)
                    {
                        throw new TypeLoadException("Could not create controller: " + RebelCmsController.GetControllerName(t.ControllerType));
                    }
            ...
            foreach (var treeController in BackOfficeRequestContext.RegisteredComponents
                .TreeControllers)
            {
                errorBuilder.AppendFormat("• {0} from Assembly {1} found in folder {2}\n", RebelCmsController.GetControllerName(treeController.Metadata.ComponentType), treeController.Metadata.ComponentType.Assembly, treeController.Metadata.PluginDefinition.PackageFolderPath);
            }
