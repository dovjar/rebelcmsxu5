...
namespace RebelCms.Cms.Web.IO
{
    ...
    public class DocumentTypeIconFileResolver : SpriteIconFileResolver
    {
        ...
        public DocumentTypeIconFileResolver(HttpServerUtilityBase server, RebelCmsSettings settings, UrlHelper url)
            : base(new DirectoryInfo(server.MapPath(settings.RebelCmsFolders.DocTypeIconFolder)), url)
        {
        }
