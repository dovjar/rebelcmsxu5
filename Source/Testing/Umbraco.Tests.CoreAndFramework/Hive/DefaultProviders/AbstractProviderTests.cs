...
using FixedEntities = Umbraco.Framework.Security.Model.FixedEntities;
...
namespace Umbraco.Tests.CoreAndFramework.Hive.DefaultProviders
{
    ...
    public abstract class AbstractProviderTests : AbstractEntityOnlyProviderTests
    {
        ...
        public void Committing_Both_Entity_And_Schema_Results_In_No_Group_Overlap()
        {
            ...
            var userProfile = new UserProfile() { Id = new HiveId(userGuidForDebugging) };
            var actualGroups =
            ...
            {
                writer.EntityRepository.AddOrUpdate(new SystemRoot());
                writer.EntityRepository.AddOrUpdate(FixedEntities.UserProfileVirtualRoot);
                writer.EntityRepository.AddOrUpdate(FixedEntities.UserGroupVirtualRoot);
                writer.EntityRepository.Schemas.AddOrUpdate(Umbraco.Framework.Security.Model.FixedSchemas.UserProfileSchema);
                writer.EntityRepository.Schemas.AddOrUpdate(Umbraco.Framework.Security.Model.FixedSchemas.UserGroup);
                writer.EntityRepository.AddOrUpdate(userProfile);
                writer.EntityRepository.AddOrUpdate(userGroup);
                writer.EntityRepository.AddRelation(Umbraco.Framework.Security.Model.FixedHiveIds.UserGroupVirtualRoot,
                                                    userGroup.Id,
            ...
            {
                var foundGroup =
                    writer.EntityRepository.GetChildren<UserGroup>(
                        ...
                        .FirstOrDefault();
                userProfile.RelationProxies.EnlistParent(foundGroup, FixedRelationTypes.UserGroupRelationType);
                writer.EntityRepository.AddOrUpdate(userProfile);
                writer.Complete();
            }
        ...
        public void Get_Entity_By_Relation_Type()
        {
            ...
            var myUserGroup = new UserGroup()
            {
                Name = "MyUser"
            };

            adminUserGroup.RelationProxies.EnlistParentById(Umbraco.Framework.Security.Model.FixedHiveIds.UserGroupVirtualRoot, FixedRelationTypes.DefaultRelationType);
            myUserGroup.RelationProxies.EnlistParentById(Umbraco.Framework.Security.Model.FixedHiveIds.UserGroupVirtualRoot, FixedRelationTypes.DefaultRelationType);

            using (var writer = ProviderSetup.UnitFactory.Create())
            {
                writer.EntityRepository.AddOrUpdate(new SystemRoot());
                writer.EntityRepository.AddOrUpdate(FixedEntities.UserGroupVirtualRoot);
                writer.EntityRepository.AddOrUpdate(adminUserGroup);
                writer.EntityRepository.AddOrUpdate(myUserGroup);
                writer.Complete();
            }
            ...
            using (var reader = ProviderSetup.UnitFactory.Create())
            {
                var userGroups = reader.EntityRepository.GetChildren<User>(FixedRelationTypes.DefaultRelationType, Framework.Security.Model.FixedHiveIds.UserGroupVirtualRoot).ToArray();
        ...
        public void Create_EntitySchema_CheckExists_ByExistsThenByGetting()
        {
            ...
            schema.AttributeDefinitions.Add(new SelectedTemplateAttributeDefinition(new AttributeGroup("mygroup", "My Group", 0))
                {
                    Alias = "test"
                });
            ...
            using (var reader = ReadonlyProviderSetup.ReadonlyUnitFactory.CreateReadonly())
            {
                ...
                var entitySchema = reader.EntityRepository.Schemas.Get<EntitySchema>(schema.Id);
                Assert.NotNull(entitySchema);
                Assert.That(entitySchema.AttributeDefinitions.FirstOrDefault(x => x.Alias == "test"), Is.Not.Null);
            }
        }
