...
namespace Umbraco.Tests.CoreAndFramework.Cms
{
    ...
    public class DataInstallTests //: AbstractPartialTrustFixture<DataInstallTests>
    {
        ...
        {
            var hiveMembershipProvider = membershipProvider as AbstractUmbracoMembershipProvider;
        ...
        private void DoCoreAssertions(IHiveManager hiveManager, int totalSchemaCount, int totalEntityCount, int totalAttributeTypeCount, int mediaSchemaCount, IEnumerable<Lazy<Permission, PermissionMetadata>> permissions, ISecurityService securityService)
        {
            ...
            using (var uow = hiveManager.OpenReader<IContentStore>())
            {
                ...
                Assert.IsTrue(uow.Repositories.Schemas.Exists<EntitySchema>(Umbraco.Framework.Security.Model.FixedSchemas.MembershipUserSchema.Id));
                Assert.IsTrue(uow.Repositories.Schemas.Exists<EntitySchema>(Umbraco.Framework.Security.Model.FixedSchemas.UserGroup.Id));
