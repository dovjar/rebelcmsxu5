...
namespace RebelCms.Tests.Cms.Stubs
{
    public class FakeRenderModelFactory : IRenderModelFactory
    {
        private readonly IRebelCmsApplicationContext _application;

        public static FakeRenderModelFactory CreateWithApp()
        {
            return new FakeRenderModelFactory(new FakeRebelCmsApplicationContext());
        }

        public FakeRenderModelFactory(IRebelCmsApplicationContext application)
        {
        ...
        {
            var mock = MockRepository.GenerateMock<IRebelCmsRenderModel>();
