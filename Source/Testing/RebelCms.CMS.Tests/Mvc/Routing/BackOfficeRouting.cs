...
namespace RebelCms.Tests.Cms.Mvc.Routing
{
    ...
    public class BackOfficeRouting : RoutingTest
    {
        ...
        {
            RouteTable.Routes.GetDataForRoute("~/RebelCms/Trees/ApplicationTree/Search")
        ...
        {
            RouteTable.Routes.GetDataForRoute("~/RebelCms").ShouldMapTo<DefaultController>(x => x.App("content"));
        }
        ...
        public void BackOfficeRouting_Default_Standard_Route()
        {
            ...
            RouteTable.Routes.GetDataForRoute("~/RebelCms/Default/Login").ShouldMapTo<DefaultController>(x => x.Login(LoginDisplayType.StandardPage));
        }

        [Test]
        [Category(TestOwner.CmsBackOffice.Routing)]
        public void BackOfficeRouting_Default_RebelCms_Editors_Routes()
        {

            var entityUri = new HiveId(123);

            RouteTable.Routes.GetDataForRoute("~/RebelCms/Editors/ContentEditor")
                .ShouldMapTo<ContentEditorController>(x => x.Dashboard(null));
            RouteTable.Routes.GetDataForRoute("~/RebelCms/Editors/ContentEditor/Edit/" + entityUri)
                .ShouldMapTo<ContentEditorController>(x => x.Edit(entityUri));

            RouteTable.Routes.GetDataForRoute("~/RebelCms/Editors/DataTypeEditor")
                .ShouldMapTo<DataTypeEditorController>(x => x.Dashboard(null));
            RouteTable.Routes.GetDataForRoute("~/RebelCms/Editors/DataTypeEditor/Edit/" + entityUri)
                .ShouldMapTo<DataTypeEditorController>(x => x.Edit(new HiveId(123)));

            RouteTable.Routes.GetDataForRoute("~/RebelCms/Editors/DocumentTypeEditor")
        ...
        public void BackOfficeRouting_Custom_Plugin_Editors_Routes()
        {
            ...
            var contentEditorDashboard = "~/RebelCms/TestPackage/ContentEditor";
            var mediaEditorDashboard = "~/RebelCms/TestPackage/MediaEditor";
            var contentEditorEdit = "~/RebelCms/TestPackage/ContentEditor/Edit";
            var mediaEditorEdit = "~/RebelCms/TestPackage/MediaEditor/Edit";
            //var contentEditorEditWithId = "~/RebelCms/TestPackage/ContentEditor/Edit/" + new HiveId(123); ;
            //var mediaEditorEditWithId = "~/RebelCms/TestPackage/MediaEditor/Edit/" + new HiveId(123); ;
        ...
        {
            var surfaceRoute1 = "~/RebelCms/TestPackage/CustomSurface";
            var surfaceRoute2 = "~/RebelCms/TestPackage/BlahSurface";
            var surfaceRouteWithCustomAction1 = "~/RebelCms/TestPackage/CustomSurface/SomeAction";
            var surfaceRouteWithCustomAction2 = "~/RebelCms/TestPackage/BlahSurface/AnotherAction";
        ...
        [Test]
        [Category(TestOwner.CmsBackOffice.Routing)]
        public void BackOfficeRouting_Default_RebelCms_Tree_Routes()
        {
            RouteTable.Routes.GetDataForRoute("~/RebelCms/Trees/ApplicationTree/Index")
                .ShouldMapTo<ApplicationTreeController>(x => x.Index("content"));
            RouteTable.Routes.GetDataForRoute("~/RebelCms/Trees/ApplicationTree/Index/media")
                .ShouldMapTo<ApplicationTreeController>(x => x.Index("media"));

            //test the default action/id
            RouteTable.Routes.GetDataForRoute("~/RebelCms/Trees/ContentTree")
                .ShouldMapTo<ContentTreeController>(x => x.Index(HiveId.Empty, null));
            //test a full path
            var e2 = new HiveId(123);
            RouteTable.Routes.GetDataForRoute("~/RebelCms/Trees/ContentTree/Index/" + e2)
                .ShouldMapTo<ContentTreeController>(x => x.Index(new HiveId(123), null));
            //test a second controller type
            RouteTable.Routes.GetDataForRoute("~/RebelCms/Trees/MediaTree")
            ...
            RouteTable.Routes.GetDataForRoute("~/RebelCms/Trees/MediaTree/Index/" + e3)
                .ShouldMapTo<MediaTreeController>(x => x.Index(new HiveId(456), null));
            
            ////test a default id
            //var guid = Guid.NewGuid();
            //RouteTable.Routes.GetDataForRoute("~/RebelCms/Trees/DataTypeTree/Index/" + guid.ToString("N"))
        ...
        {
            var contentUrl = "~/RebelCms/TestPackage/ContentTree";
            var mediaUrl = "~/RebelCms/TestPackage/MediaTree";
        ...
        public void BackOfficeRouting_Ensure_Default_Editor_Url_Structures()
        {
            ...
            var contentControllerName = RebelCmsController.GetControllerName(contentEditor.GetType());
            var contentControllerId = RebelCmsController.GetControllerId<EditorAttribute>(contentEditor.GetType());

            var dataTypeEditor = new DataTypeEditorController(new FakeBackOfficeRequestContext());
            var dataTypeControllerName = RebelCmsController.GetControllerName(dataTypeEditor.GetType());
            var dataTypeControllerId = RebelCmsController.GetControllerId<EditorAttribute>(dataTypeEditor.GetType());

            var docTypeEditor = new DocumentTypeEditorController(new FakeBackOfficeRequestContext());
            var docTypeControllerName = RebelCmsController.GetControllerName(docTypeEditor.GetType());
            var docTypeControllerId = RebelCmsController.GetControllerId<EditorAttribute>(docTypeEditor.GetType());
            
            const string customAction = "Index";
            const string defaultAction = "Dashboard";
            const int id = -1;
            const string area = "RebelCms";
            ...
            Assert.AreEqual(string.Format("/RebelCms/Editors/{0}", contentControllerName), contentEditorDefaultUrl);
            Assert.AreEqual(string.Format("/RebelCms/Editors/{0}", dataTypeControllerName), dataTypeEditorDefaultUrl);
            Assert.AreEqual(string.Format("/RebelCms/Editors/{0}", docTypeControllerName), docTypeEditorDefaultUrl);
            Assert.AreEqual(string.Format("/RebelCms/Editors/{0}/{1}/{2}", contentControllerName, customAction, id), contentEditorCustomUrl);
            Assert.AreEqual(string.Format("/RebelCms/Editors/{0}/{1}/{2}", dataTypeControllerName, customAction, id), dataTypeEditorCustomUrl);
            Assert.AreEqual(string.Format("/RebelCms/Editors/{0}/{1}/{2}", docTypeControllerName, customAction, id), docTypeEditorCustomUrl);
        }
        ...
        public void BackOfficeRouting_Ensure_Plugin_Editor_Url_Structures()
        {
            ...
            var mediaControllerName = RebelCmsController.GetControllerName(mediaEditorController.GetType());
            var mediaControllerId = RebelCmsController.GetControllerId<EditorAttribute>(mediaEditorController.GetType());
            ...
            //Assert.AreEqual(string.Format("/RebelCms/TestPackage/Editors/{0}/{1}", contentControllerId.ToString("N"), contentControllerName), contentEditorDefaultUrl);
            //Assert.AreEqual(string.Format("/RebelCms/TestPackage/Editors/{0}/{1}", mediaControllerId.ToString("N"), mediaControllerName), dataTypeEditorDefaulturl);
            //Assert.AreEqual(string.Format("/RebelCms/TestPackage/Editors/{0}/{1}/{2}/{3}", contentControllerId.ToString("N"), contentControllerName, customAction, id), contentEditorCustomUrl);
            //Assert.AreEqual(string.Format("/RebelCms/TestPackage/Editors/{0}/{1}/{2}/{3}", mediaControllerId.ToString("N"), mediaControllerName, customAction, id), dataTypeEditorCustomurl);

            Assert.AreEqual(string.Format("/RebelCms/TestPackage/{0}", contentControllerName), contentEditorDefaultUrl);
            Assert.AreEqual(string.Format("/RebelCms/TestPackage/{0}", mediaControllerName), dataTypeEditorDefaulturl);
            Assert.AreEqual(string.Format("/RebelCms/TestPackage/{0}/{1}/{2}", contentControllerName, customAction, id), contentEditorCustomUrl);
            Assert.AreEqual(string.Format("/RebelCms/TestPackage/{0}/{1}/{2}", mediaControllerName, customAction, id), dataTypeEditorCustomurl);
        }
        ...
        public void BackOfficeRouting_Ensure_Default_Tree_Url_Structures()
        {
            ...
            var contentControllerName = RebelCmsController.GetControllerName(contentTree.GetType());
            var contentControllerId = RebelCmsController.GetControllerId<TreeAttribute>(contentTree.GetType());

            var dataTypeTree = new DataTypeTreeController(new FakeBackOfficeRequestContext());
            var dataTypeControllerName = RebelCmsController.GetControllerName(dataTypeTree.GetType());
            var dataTypeControllerId = RebelCmsController.GetControllerId<TreeAttribute>(dataTypeTree.GetType());
            ...
            Assert.AreEqual(string.Format("/RebelCms/Trees/{0}", contentControllerName), contentTreeDefaultUrl);
            Assert.AreEqual(string.Format("/RebelCms/Trees/{0}", dataTypeControllerName), dataTypeTreeDefaultUrl);
            Assert.AreEqual(string.Format("/RebelCms/Trees/{0}/{1}/{2}", contentControllerName, action, HttpUtility.UrlEncode(customId.ToString())), contentTreeCustomUrl);
            Assert.AreEqual(string.Format("/RebelCms/Trees/{0}/{1}/{2}", dataTypeControllerName, action, HttpUtility.UrlEncode(customId.ToString())), dataTypeTreeCustomUrl);
        }
        ...
        public void BackOfficeRouting_Ensure_Plugin_Tree_Url_Structures()
        {
            ...
            var mediaTreeControllerName = RebelCmsController.GetControllerName(mediaTree.GetType());
            var mediaTreeControllerId = RebelCmsController.GetControllerId<TreeAttribute>(mediaTree.GetType());
            ...
            //Assert.AreEqual(string.Format("/RebelCms/TestPackage/Trees/{0}/{1}", contentControllerId.ToString("N"), contentControllerName), contentTreeDefaultUrl);
            //Assert.AreEqual(string.Format("/RebelCms/TestPackage/Trees/{0}/{1}", mediaTreeControllerId.ToString("N"), mediaTreeControllerName), mediaTreeDefaultUrl);
            //Assert.AreEqual(string.Format("/RebelCms/TestPackage/Trees/{0}/{1}/{2}/{3}", contentControllerId.ToString("N"), contentControllerName, action, customId), contentTreeCustomUrl);
            //Assert.AreEqual(string.Format("/RebelCms/TestPackage/Trees/{0}/{1}/{2}/{3}", mediaTreeControllerId.ToString("N"), mediaTreeControllerName, action, customId), mediaTreeCustomUrl);

            Assert.AreEqual(string.Format("/RebelCms/TestPackage/{0}", contentControllerName), contentTreeDefaultUrl);
            Assert.AreEqual(string.Format("/RebelCms/TestPackage/{0}", mediaTreeControllerName), mediaTreeDefaultUrl);
            Assert.AreEqual(string.Format("/RebelCms/TestPackage/{0}/{1}/{2}", contentControllerName, action, customId), contentTreeCustomUrl);
            Assert.AreEqual(string.Format("/RebelCms/TestPackage/{0}/{1}/{2}", mediaTreeControllerName, action, customId), mediaTreeCustomUrl);
        }
