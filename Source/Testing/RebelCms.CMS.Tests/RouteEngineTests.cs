...
namespace RebelCms.Tests.Cms
{
    ...
    public class RouteEngineTests : StandardWebTest
    {
        ...
        protected RebelCms.Cms.Web.Context.IBackOfficeRequestContext GetBackOfficeRequestContext(Uri uri)
        {
            var httpContext = new FakeHttpContextFactory(uri).HttpContext;
            return new FakeBackOfficeRequestContext(RebelCmsApplicationContext, new DefaultRoutingEngine(RebelCmsApplicationContext, httpContext));
        }

        protected GroupUnitFactory GetHive()
        {
            return RebelCmsApplicationContext.Hive.GetWriter<IContentStore>();
        }
