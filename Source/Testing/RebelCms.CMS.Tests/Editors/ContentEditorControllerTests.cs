...
namespace RebelCms.Tests.Cms.Editors
{
    ...
    public class ContentEditorControllerTests : AbstractContentControllerTest
    {
        ...
        public void ContentEditorControllerTests_Content_Saved()
        {
            ...
            using (var uow = RebelCmsApplicationContext.Hive.OpenReader<IContentStore>())
            {
                ...
                var contentViewModel = RebelCmsApplicationContext.FrameworkContext.TypeMappers.Map<EntitySnapshot<TypedEntity>, ContentEditorModel>(snapshot);
        ...
        private bool ActionIsSecuredByPermission(Controller controller, string actionName, string permissionId)
        {
            ...
            var attributes = methodInfo.GetCustomAttributes(typeof(RebelCmsAuthorizeAttribute), true);
            if (attributes.Length == 0)
                return false;

            return attributes.Cast<RebelCmsAuthorizeAttribute>().Any(x => x.Permissions.Length > 0 && x.Permissions.Contains(permissionId));
        }
