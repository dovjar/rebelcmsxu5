...
namespace RebelCms.Tests.CoreAndFramework.Config
{
    [TestFixture]
    public class DeepConfigManagerTests : AbstractPartialTrustFixture<DeepConfigManagerTests>
    {
        ...
        public void DeepConfigManager_Serialize_Provider_Config_Section()
        {
            ...
            DeepConfigManager.SerializeProviderConfigSection(config, section, "RebelCms/persistenceProviderSettings/nhibernate", true);
            ...
            Assert.AreEqual("RebelCms", config.Root.Elements().First().Elements().First().Attributes().First().Value);
            ...
            Assert.AreEqual("RebelCms", config.Root.Elements().Last().Name.LocalName);
