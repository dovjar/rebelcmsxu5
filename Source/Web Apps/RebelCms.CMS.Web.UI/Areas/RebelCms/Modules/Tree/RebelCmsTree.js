...
/// <reference path="../../Scripts/RebelCms.System/RebelCmsUtils.js" />
/// <reference path="../../Scripts/RebelCms.System/WindowManager.js" />

RebelCms.System.registerNamespace("RebelCms.Controls");

(function ($, Base) {

    //tree class
    RebelCms.Controls.RebelCmsTree = Base.extend({
        ...
        _debug: function(msg, isErr) {
            ...
            window.__debug__(msg, "RebelCmsTree", isErr);
        },
        ...
        syncNode: function(nodeParam, callback, nodeSelectType) {
            ...
            if (!(nodeParam instanceof jQuery)) {
                if (!$.isArray(nodeParam)) {
                    var nodeId = RebelCms.Utils.urlDecode(nodeParam);
...
//                    }
        ...
        loadApp: function(srcUrl, callback) {
            ...
            this._opts.jsonUrl = RebelCms.Utils.urlDecode(srcUrl);
        ...
        getPluginNames: function() {
            ...
            if (this._opts.showContext) {
                return ["themes", "json_data", "ui", "RebelCmscontextmenu", "RebelCmsnodeformatter", "applicationpersistence"];                    
            }
            else {
                return ["themes", "json_data", "ui", "RebelCmsnodeformatter"];
            }
        },
            ...
            if (editorPath) {
                ...
                if (editorPath.startsWith("/") || editorPath.startsWith("http:") || editorPath.startsWith("https:")) {
                    ...
                    if (nodeSelectType == "implicit") {                       
                        ...
                        // || $(RebelCms.System.WindowManager.getInstance().contentFrame()).data("url") == editorPath
                        // however $(RebelCms.System.WindowManager.getInstance().contentFrame()).data("url") was returning undefined after a page refresh (in Chrome and FF)
                        
                        if (RebelCms.System.WindowManager.getInstance().contentFrame().location.pathname == editorPath) {
                            return;
                        }
                    }
                    
                    RebelCms.System.WindowManager.getInstance().contentFrame(editorPath);

                    //raise an event!
                    var args = { node: $node, url: editorPath };
                    $(this).trigger("nodeClicked.RebelCmsTree", [args]);
                }
                else {
        ...
        init: function(manuallyCreate) {
            ...
            if (!this._opts.jqueryElem.attr("id")) this._opts.jqueryElem.attr("id", "UmbTree_" + RebelCms.Utils.generateRandom());
        ...
        createJsTree: function(cb) {
            ...
            this._opts.jqueryElem
                ...
                .bind("loaded.jstree", function(event, data) {      
                    ...
                    $(_this).trigger("appLoaded.RebelCmsTree", data);
                })
    ...
    //jquery extension
    $.fn.RebelCmsTree = function(opts, manualInit) {
        /// <summary>jQuery extension to create the RebelCms tree</summary>

        return this.each(function() {
            var _tree = new RebelCms.Controls.RebelCmsTree($.extend({ jqueryElem: $(this) }, opts));
            _tree.init(manualInit);
        });
    };

    $.fn.RebelCmsTreeApi = function() {
        /// <summary>exposes the RebelCms Tree api for the selected object</summary>
        if ($(this).length != 1) {
            throw "RebelCmsTreeAPI selector requires that there be exactly one control selected, this selector returns " + $(this).length;
        };
