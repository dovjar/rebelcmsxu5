...
(function ($, Base) {
    ...
    RebelCms.UI.PermissionsGrid = Base.extend({
        ...
        constructor: function (el, o) {
            ...
            this._viewModel = {
                userGroupPermissionsModel: new RebelCms.UI.PermissionsGridUserGroupPermissionsModel(this._opts.userGroupPermissionsModel),
    ...
    // View models
    RebelCms.UI.PermissionsGridUserGroupPermissionsModel = function (val) {
        return {
            userGroupId: ko.observable(val.userGroupId),
            userGroupName: ko.observable(val.userGroupName),
            permissions: ko.observableArray(new RebelCms.UI.PermissionsGridPermissionStatusModelCollection(val.permissions)),
            allowAll: function () {
    ...
    RebelCms.UI.PermissionsGridPermissionStatusModelCollection = function(val) {
        var permissions = [];
        for(var i = 0; i < val.length; i++) {
            permissions.push(new RebelCms.UI.PermissionsGridPermissionStatusModel(val[i]));
        }
        return permissions;
    };
    
    RebelCms.UI.PermissionsGridPermissionStatusModel = function(val) {
        return {
    ...
    $.fn.permissionsGrid = function(o) {
        ...
        return $(this).each(function() {
            var treePicker = new RebelCms.UI.PermissionsGrid(this, _opts);
    ...
    var gridRowTemplate = "<tr data-id=\"${ permissionId }\">\
        <td>\
            ...
            <span data-bind=\"visible: initialStatus != status()\" title=\"Value has changed.\"><img src=\"../../../../areas/RebelCms/content/images/bullet_error.png\" alt=\"Alert\" /></span>\
        </td>\
