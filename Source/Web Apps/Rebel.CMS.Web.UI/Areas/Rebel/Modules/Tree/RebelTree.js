...
/// <reference path="../../Scripts/Rebel.System/RebelUtils.js" />
/// <reference path="../../Scripts/Rebel.System/WindowManager.js" />

Rebel.System.registerNamespace("Rebel.Controls");

(function ($, Base) {

    //tree class
    Rebel.Controls.RebelTree = Base.extend({
        ...
        _debug: function(msg, isErr) {
            ...
            window.__debug__(msg, "RebelTree", isErr);
        },
        ...
        syncNode: function(nodeParam, callback, nodeSelectType) {
            ...
            if (!(nodeParam instanceof jQuery)) {
                if (!$.isArray(nodeParam)) {
                    var nodeId = Rebel.Utils.urlDecode(nodeParam);
                    ...
                    this.reloadSingleNode(nodeParam, callback, nodeSelectType);
                }
                else {
...
//                    }
                    ...
                    if (deepestNodeId.equals(paths[paths.length - 1])) {
                        this.reloadSingleNode(deepestNode, callback, nodeSelectType);
                    }
                    else {
        ...
        loadApp: function(srcUrl, callback) {
            ...
            this._opts.jsonUrl = Rebel.Utils.urlDecode(srcUrl);
        ...
        getPluginNames: function() {
            ...
            if (this._opts.showContext) {
                return ["themes", "json_data", "ui", "rebelcontextmenu", "rebelnodeformatter", "applicationpersistence"];                    
            }
            else {
                return ["themes", "json_data", "ui", "rebelnodeformatter"];
            }
        },
            ...
            if (editorPath) {
                ...
                if (editorPath.startsWith("/") || editorPath.startsWith("http:") || editorPath.startsWith("https:")) {
                    ...
                    if (nodeSelectType == "implicit") {                       
                        ...
                        // || $(Rebel.System.WindowManager.getInstance().contentFrame()).data("url") == editorPath
                        // however $(Rebel.System.WindowManager.getInstance().contentFrame()).data("url") was returning undefined after a page refresh (in Chrome and FF)
                        
                        if (Rebel.System.WindowManager.getInstance().contentFrame().location.pathname == editorPath) {
                            return;
                        }
                    }
                    
                    Rebel.System.WindowManager.getInstance().contentFrame(editorPath);

                    //raise an event!
                    var args = { node: $node, url: editorPath };
                    $(this).trigger("nodeClicked.rebelTree", [args]);
                }
                else {
        ...
        init: function(manuallyCreate) {
            ...
            if (!this._opts.jqueryElem.attr("id")) this._opts.jqueryElem.attr("id", "UmbTree_" + Rebel.Utils.generateRandom());
        ...
        createJsTree: function(cb) {
            ...
            this._opts.jqueryElem
                ...
                .bind("loaded.jstree", function(event, data) {      
                    ...
                    $(_this).trigger("appLoaded.rebelTree", data);
                })
    ...
    //jquery extension
    $.fn.rebelTree = function(opts, manualInit) {
        /// <summary>jQuery extension to create the rebel tree</summary>

        return this.each(function() {
            var _tree = new Rebel.Controls.RebelTree($.extend({ jqueryElem: $(this) }, opts));
            _tree.init(manualInit);
        });
    };

    $.fn.rebelTreeApi = function() {
        /// <summary>exposes the Rebel Tree api for the selected object</summary>
        if ($(this).length != 1) {
            throw "RebelTreeAPI selector requires that there be exactly one control selected, this selector returns " + $(this).length;
        };
