...
Rebel.System.registerNamespace("Rebel.Utils");

(function ($, Base) {

    Rebel.Utils = Base.extend(null, {
        ...
        ctrlClick: function(key, callback, args, $validationElement) {
            ...
            $(document).keydown(function(e) {
                ...
                if (e.keyCode == key.charCodeAt(0) && e.ctrlKey) {
                    if (Rebel.Utils.isJquery($validationElement) && $.validator) {
        ...
        bindShortcuts: function(selector) {
            ...
            $(selector).find("*[data-shortcut]").each(function() {
                ...
                if (parts[0] == "ctrl")
                    Rebel.Utils.ctrlClick(parts[1], function(e) {
                        $this.trigger(triggerEvent);
                    }, null, $validationSelector);
                else if (parts[0] == "alt")
                    Rebel.Utils.altClick(parts[1], function(e) {
                        $this.trigger(triggerEvent);
                    }, null, $validationSelector);
                else if (parts[0] != "shift")
                    Rebel.Utils.shiftClick(parts[1], function(e) {
                        $this.trigger(triggerEvent);
                    }, null, $validationSelector);
                else
        ...
        preventDoublePostback: function () {
            var form = $(this);
            var btns = $("input:submit", form);

            if (!form.valid()) {
                // allow user to correct validation errors and re-submit
                btns.unbind("click.preventDoublePostback");
            } else {
                btns.bind("click.preventDoublePostback", function (e) {
                    e.preventDefault();
                });
            }
        },
