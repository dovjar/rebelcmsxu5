...
(function ($, Base) {
    ...
    $.fn.rebelApplication = function(o) {
        return $(this).each(function() {
            var app = new Rebel.System.RebelApplication(o);
            $(this).data("api", app);
        });
    };

    $.fn.rebelApplicationApi = function() {
        ...
        if ($(this).data("api") == null) {
            throw "The matching element had not been bound to an rebelApplication";
        }
        return $(this).data("api");
    };

    Rebel.System.RebelApplication = Base.extend({
        ...
        constructor: function (o) {
            ...
            this._searchBoxApi = this._searchInput.rebelSearchBoxApi();
            ...
            $(this._opts.mainTree).bind("nodeClicked.rebelTree", function(e, args) {
            ...
            $(this._opts.mainTree).bind("appLoaded.rebelTree", function(e, args) {
        ...
        _toggleSearchBox: function($rootNodes, rebelTree) {
            if ($rootNodes.length != 1) {
                this._searchInput.fadeOut(200);
            }
            else {
                var metaData = rebelTree.getNodeMetaData($rootNodes);
        ...
        loadDashboard: function (appAlias) {
            ...
            //show the dashboard
            this._opts.windowMgr.contentFrame(Rebel.Utils.urlDecode(dashboard));
        },
        ...
        switchApp: function(appAlias) {
            ...
            this._opts.mainTree.loadApp(src, function(data) {
                //need to check the callback to see if its selecting any persisted nodes (this stuff is stored using the Rebel ApplicationPersistence jstree plugin)
            ...
            window.top.document.title = tray.text() + ' - Rebel CMS - ' + window.top.location.hostname.toLowerCase().replace('www', '');
        },

        detectLoginPage: function(pageHtml) {
            ///<summary>Checks if the pageHtml is the html for the login page</summary>

            if (pageHtml.search( /<meta\s+name="rebel-page-type"\s+content="rebel-login-page"/ ) != -1) {
                return true;
            }
            return false;
        },
        ...
        handleAuthTimeout: function (x, t, e, useModal) {
            ...
            if (this.detectLoginPage(x.responseText)) {
                if (!useModal) {
                    ...
                    Rebel.System.WindowManager.getInstance().showModal({ id: "login-dialog", isGlobal: true, title: "Login Expired", modalClass: "login", forceContentInIFrame: true, contentUrl: this._opts.loginUrl, showClose: false });   
                }                
                return true;
            }
            return false;
        ...
        handleAjaxException: function(x, t, e) {
            ...
            //so if the responseText has a meta tag: rebel-page-type with a value of: rebel-login-page we know its the login page                
            if (!this.handleAuthTimeout(x, t, e, true)) {                
                Rebel.System.WindowManager.getInstance().showModal({ isGlobal: true, title: "Server error", modalClass: "exception", forceContentInIFrame: true, content: x.responseText });
            }
        }
